...
def q_learning(N_steps, test_steps):
    ...
    rewards_table = np.zeros((S, 4))
    dynamics_table = np.zeros((S, 4, S))
    # Initialize bounds
    L = np.full((S, 4), -np.inf)
    U = np.full((S, 4), np.inf)
...
np.save(r"C:\Users\kevin\OneDrive\Desktop\TMLR 2025\Exp 1 Fixed MDP FIxed R\highway-city-world\memorized rewards\QM+SFQL\QM+SFQL+SQB_ours_MR_learning.npy", rewards_run)
