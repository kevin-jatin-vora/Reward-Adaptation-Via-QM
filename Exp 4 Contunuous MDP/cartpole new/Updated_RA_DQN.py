...
with open('Q_UB_test.pickle', 'rb') as handle:
    ub = pickle.load(handle)
ub = defaultdict(default_value, ub)

with open('Q_LB_test.pickle', 'rb') as handle:
...
info=[]
final_actions=set(list(range(2)))
prune={}
state_action={}
...
for i in s:
    alist=[]
    for action_l in range(2):
        for action_u in range(2):
...
remaining = sum([1 for i in [len(state_action[i]) for i in state_action.keys()] if i==1])
# print(((len(s))*2-sum([len(state_action[i]) for i in state_action.keys() if i!=0]))/((len(s))*2))
print(1-(len(s)*2-remaining)/(len(s)*2))
...
class Agent():
    ...
    def act(self, state, eps, train_flag):
        ...
        #     return np.argmax(action_values.cpu().data.numpy())
            ...
            action =  np.argmax(action_values.cpu().data.numpy())
            qm_actions = list(state_action[tuple(features)])
            # print(qm_action)
...
def dqn_fixed_steps(n_steps=500000, max_t=475, eps_start=1.0, eps_end=0.01, eps_decay=0.997, test_interval=300, test_runs=10):
    ...
    for step in range(1, n_steps+1):
        ...
        if step % 100000 == 0:
            # print('\rStep {}\tAverage Score: {:.2f}'.format(step, np.mean(scores_window)), end="")
            torch.save(agent.qnetwork_local.state_dict(), 'checkpoint_RA_test2.pth')
...
reward_run[:5] = np.load("dqn_RA_new_test.npy")
for i in range(5,run):
    ...
    reward_run[i]=scores
    np.save("dqn_RA_new_test2.npy", reward_run)
    fig = plt.figure()
