...
class Agent():
    ...
    def act(self, state, eps=0.):
        ...
        if random.random() > eps:
            ...
            action_index = np.argmin(action_values.cpu().data.numpy())
...
def dqn(n_episodes=2000, max_t=500, eps_start=1.0, eps_end=0.01, eps_decay=0.997):
    """Deep Q-Learning.
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        # while True:
            ...
            # reward = -abs(vel) if abs(vel)<=0.2 else -25
            # reward = reward/3
            next_state = np.array(list(env.getGameState().values()))
            reward = r
            if(next_state[0]==3.5):
                reward -= 1
            if(next_state[0]==44.5):
                reward += 1
            done = env.game_over()
