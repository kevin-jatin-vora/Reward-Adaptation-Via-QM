...
class MDPGenerator:
    ...
    def generate_transitions_and_rewards(self, max_bf):
        ...
        # Randomly select terminal states
        ...
        for i in self.terminal_state:
            x.append(np.where(self.transitions[:,:,i] == np.max(self.transitions[:,:,i]))[0][0])
            y.append(np.where(self.transitions[:,:,i] == np.max(self.transitions[:,:,i]))[1][0])

        # Update rewards for the selected states
        self.rewards[x[0], y[0], self.terminal_state[0]] = 1
        self.rewards2[x[0], y[0], self.terminal_state[0]] = -1

        self.rewards[x[1], y[1], self.terminal_state[1]] = -1
        self.rewards2[x[1], y[1], self.terminal_state[1]] = 1

        self.rewards[x[2], y[2], self.terminal_state[2]] = 0.6
        self.rewards2[x[2], y[2], self.terminal_state[2]] = 0.6
...
for i in range(3):
    # os.mkdir(str(i))
    num_actions = np.random.randint(9, 20)
    num_states = np.random.randint(num_actions, 80)
        ...
        for s in range(num_states):
            for a in range(num_actions):
                ...
                if(bf!=1):
                    ...
                    existing_states = tracker[(s, a, bf-4)]
