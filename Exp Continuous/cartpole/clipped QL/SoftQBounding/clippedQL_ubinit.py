...
class BoundedDQNAgent:
    def __init__(self, state_size, action_size, seed=0, gamma=0.99, lr=1e-3, tau=1e-3,
                 buffer_size=int(1e5), batch_size=64, prior_update_interval=1000, beta=0.019419325239076097, eta=1):
        self.gamma = gamma
        self.tau = tau
        self.batch_size = batch_size
        ...
        path = r"C:\Users\kevin\Downloads\Continuous domain new attempt 2025\cartpole\behaviors\regression idea for neighbours\checkpoint_b1.pth"
...
def train_agent(env, agent, n_steps=100000, max_t=200, eps_start=1.0, eps_end=0.01, eps_decay=0.995, test_interval=1000, test_runs=5):
    scores = []
    ...
    for step in range(1, n_steps + 1):
        ...
        if done or (step % max_t == 0):
            state = env.reset()[0]
            scores_window.append(score)
            print(f"\rStep {step}\tAvg Score: {np.mean(scores_window):.2f}", end="")
            score = 0
            eps = max(eps_end, eps_decay * eps)

        if step % test_interval == 0:
            avg_test_score = test_agent(env, agent, test_runs, max_t)
            scores.append(avg_test_score)
            # print(f"\nTest at Step {step}: Avg Score = {avg_test_score:.2f}")

    return scores

def test_agent(env, agent, test_runs=10, max_t=200):
    test_scores = []
...
if __name__ == "__main__":
    ...
    for i in range(run):
        print("########################### "+str(i))
        agent = BoundedDQNAgent(state_size=4, action_size=2, seed=i)
        scores = train_agent(env,agent,n_steps=N_step, max_t=200, eps_start=1.0, eps_end=0.01, eps_decay=0.9905, test_interval=test_steps, test_runs=5) #dqn(n_episodes)
        reward_run[i]=scores
        np.save("sqb_b1_init2.npy", reward_run)
        fig = plt.figure()
