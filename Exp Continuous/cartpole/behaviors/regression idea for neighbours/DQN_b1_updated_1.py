...
#         self.delta_buffers = [deque(maxlen=MIN_SAMPLES_FOR_UPDATE*2) for _ in range(action_dim)]
#         self.cluster_stats = [{
#             'min': np.zeros((N_CLUSTERS, state_dim)),
#             'max': np.zeros((N_CLUSTERS, state_dim))
#         } for _ in range(action_dim)]
#         self.steps_since_update = 0
...
#         states = np.array(self.state_buffers[action_idx])
#         deltas = np.array(self.delta_buffers[action_idx])
#         self.kmeans[action_idx].partial_fit(states)
#         labels = self.kmeans[action_idx].predict(states)
        
#         for cluster in range(N_CLUSTERS):
#             mask = (labels == cluster)
#             if np.any(mask):
#                 cluster_deltas = deltas[mask]
#                 self.cluster_stats[action_idx]['min'][cluster] = np.min(cluster_deltas, axis=0)
#                 self.cluster_stats[action_idx]['max'][cluster] = np.max(cluster_deltas, axis=0)
...
#         cluster = self.kmeans[action_idx].predict(state.reshape(1, -1))[0]
#         return (self.cluster_stats[action_idx]['min'][cluster],
#                 self.cluster_stats[action_idx]['max'][cluster])
...
#             joblib.dump({
#                 'kmeans': self.kmeans[action_idx],
#                 'stats': self.cluster_stats[action_idx],
#                 'sample_count': len(self.state_buffers[action_idx])
#             }, f"{path}/action_{action_idx}.pkl")
...
def dqn(n_episodes=15000, max_t=475, eps_start=1.0, eps_end=0.01, eps_decay=0.995):
...
if __name__ == "__main__":
    scores, clusterer = dqn(n_episodes=15000)
